        ...
        if candidates:
            if st.button("Run Ranking Pipeline"):
                ...
                for idx, feat in enumerate(features_list):
                    ...
                    # Override weights dynamically if adjusted in UI
                    # We inject UI weights into score_candidate by modifying global/local score factors if needed,
                    # but here we can just run the standard scoring function
                    ...
                    # Generate reasoning string (imported logic or custom wrapper)
                    from rank import generate_reasoning
                    reason = generate_reasoning(feat, 50, sim) # Pass middle-tier rank for default UI display
                ...
                # Sort and rank (Break ties by candidate_id ascending)
                scored_candidates.sort(key=lambda x: (-round(x["score"], 4), x["candidate_id"]))
                
                # Add ranks
                for i, item in enumerate(scored_candidates):
                    item["rank"] = i + 1
                    # Re-run reasoning with exact rank
                    # Find matching feature record
